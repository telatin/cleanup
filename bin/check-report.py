...
controls = ["chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", "chr9", "chr10", "chr11", "chr12", "chr13", "chr14", "chr15", "chr16", "chr17", "chr18", "chr19", "chr20", "chr21", "chr22", "chrX", "chrY", "chrM"]
...
if __name__ == "__main__":
    ...
    args.add_argument("-m", "--min-ratio", help="Output file", default=0.2)
    # Add a list of strings
    args.add_argument("-c", "--controls", help="List of taxa (denominator)", nargs="*", default=controls)
    args.add_argument("-n", "--numerator", help="List of taxa (numerator)", nargs="*", default=detectable)
    args.add_argument("-d", "--debug", help="Debug mode", action="store_true")
    ...
    if total_pct == 0:
        print("No contamination found", file=args.output)
        sys.exit(0)
    ...
    if ratio < float(args.min_ratio):
        print("FAIL: Ratio is below %s" % args.min_ratio, file=sys.stderr)
        sys.exit(1)
    else:
        print("PASS: Ratio is above or equal to %s" % args.min_ratio, file=sys.stderr)
